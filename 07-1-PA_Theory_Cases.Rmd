...
2. Prep email/announcement assigning class to groups for next class
...
.center[.size50[.content-box-white[**Has the CAT been effective?**]]]
...
.size40[.content-box-white[**Why would states delegate power to an external agent?**]]

<br>

```{r, echo = FALSE, fig.align = 'center', out.width = '80%'}
knitr::include_graphics("libs/Images/07_1-youre-hired.gif")
```
...
knitr::include_graphics("libs/Images/07_1-Legalization_Fig1_box_delegation.png")
```
]
...
.center[.size40[.content-box-white[**Why would states delegate power to an external agent?**]]]
...
1. (An explicit contract makes the exact limits and expectations of the relationship clear for all parties)

2. (Limited in time or scope means the power transfer is expressly limited from the beginning)

3. (Revocable agency means the agent can be fired)
    - The power transfer is NOT permanent

4. (The agent is only an agent, with all rights and privileges attached, so long as it works for the principal under the terms the principal has elaborated)
...
### - Who has an example of an IO providing the first benefit?
...
background-image: url('libs/Images/07_1-Agent_Slack.jpg')
background-size: 100%
background-position: center
class: middle
...
- Becuase those benefits depend on minimizing agent slack
...
- Go through this list with the case study you brought to class in mind.

- Get ready to introduce your case to us as an example of one or more of these control mechanisms in action.
...
knitr::include_graphics("libs/Images/07_1-Hawkins_Lake_Tierney_Fig1.png")
```
...
.size65[.content-box-white[**Next Class**]]

```{r, echo = FALSE, fig.align = 'center', out.width = '100%'}
knitr::include_graphics("libs/Images/07-1-Assignment.png")
```
...
1. I want everyone to read "Understanding the ICC" document

2. Each of you will be assigned to a group and each group will be tasked with finding us real-world examples of one of these important roles fulfilled by the ICC.
