...
ggplot(data = d3) +
  ...
  labs(fill = "", title = "CIRIGHTS: Torture in 2022") +
...
d |>
  ...
  geom_col() +
  scale_fill_manual(values = c("#CC2908", "orange", "#4D70F0")) +
  labs(x = "", y = "Country Count", title = "Global Trends in Torture (CIRIGHTS)", fill = "") +
  theme_bw()
...
d |>
  filter(country == "United States of America") |>
  ggplot(aes(x = year, y = Torture)) +
  geom_rect(xmin = 1980, xmax = 2022, ymin = .5, ymax = 1.5, fill = "orange") +
  geom_rect(xmin = 1980, xmax = 2022, ymin = -.1, ymax = .6333, fill = "#CC2908") +
  geom_rect(xmin = 1980, xmax = 2022, ymin = 1.2666, ymax = 2.1, fill = "#4D70F0") +
  geom_line(linewidth = 2) +
  #geom_col() +
  theme_bw() +
  scale_y_continuous(breaks = 0:2) +
